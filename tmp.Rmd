...
generate-multi-conf.sh -d configs/multi/ -g genetic.simplified.multi-conf.part 0 "$OUTPUT_DIR/gen1/0"
```

- the `-d` option specifies the directory where the configuration "parts" are found. 
- `-g` is used to force the use of a specific config file for the genetic part instead of the default (see also [below](#genetic-algorithm) about the genetic process configuration). 
- The first mandatory argument is 0 for ignoring obsertvation types involving POS tags, 1 otherwise.
...
cat data/user-guide.expl1/cases.txt | sed 's:data/[^/]*/::g' >"$OUTPUT_DIR/gen1/cases.txt"
...
```
echo "$OUTPUT_DIR/gen1/0/multi-conf-files/basic.multi-conf" | train-genetic.sh "$OUTPUT_DIR/gen1" user-guide.output/input indivGenetic_
```
