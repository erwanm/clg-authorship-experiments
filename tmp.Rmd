...
source session-setup.sh
task-distrib-daemon.sh  -s 30s -p 1 -v -q 4 "$TASKS_DIR" $NCORES >"$OUTPUT_DIR/gen1.task-daemon.log" &
train-multi-stages.sh -r -P "$TASKS_DIR/mytasks" -o  '-c -s' "$OUTPUT_DIR/gen1" "$OUTPUT_DIR/gen1/input/truth.txt" "$OUTPUT_DIR/gen1/0/multi-conf-files/basic.multi-conf" indivGenetic
```
...
The test set used by the script must be a subset of the full training set, more precisely a subset of the list of cases contained in `$OUTPUT_DIR/gen1/input/truth.txt`. We simulate this in the example below by concatenating the test cases to the file `truth.txt` below:

```{bash gen.meta.init1}
cat "$OUTPUT_DIR/gen1/test-cases.gold.txt" >>"$OUTPUT_DIR/gen1/input/truth.txt"
```
...
```{bash gen.meta.init2}
cp configs/multi/meta-template.simplified.multi-conf "$OUTPUT_DIR/gen1/meta-template.multi-conf"
...
source session-setup.sh
task-distrib-daemon.sh  -s 30s -p 1 -v -q 4 "$TASKS_DIR" $NCORES >"$OUTPUT_DIR/gen1.task-daemon.log" &
train-outerCV-1fold.sh -r -P "$TASKS_DIR/mytasks" -o  '-c -s' "$OUTPUT_DIR/gen1" "$OUTPUT_DIR/gen1/input/truth.txt" "$OUTPUT_DIR/gen1/test-cases.gold.txt" "$OUTPUT_DIR/gen1/0/multi-conf-files"
```
...
Important: the input data dir is supposed to contain a file `truth.txt` containing all the training cases with their answer.
...
TODO assumes truth.txt with all cases

The script `train-top-level.sh` can be used to run an additional level of cross-validation, although it does not actually perform proper cross-validation:
...
The script receives only the previously initialized working directory, which contains the training data and multi-configuration files. A call could look like below (this is in run by the Rmd file because it's not fully compatible with the way the previous steps have been run).

```
task-distrib-daemon.sh  -s 30s -p 1 -v -q 4 "$TASKS_DIR" $NCORES >"$OUTPUT_DIR/gen1.task-daemon.log" &
train-top-level.sh -r -P "$TASKS_DIR/mytasks" -o  '-c -s' "$OUTPUT_DIR/gen1"
```
