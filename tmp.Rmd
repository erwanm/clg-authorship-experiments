...
**Important:** The "meta" config file must also be present in the directory under the name `meta-template.multi-conf`.

```{bash multi2}
source session-setup.sh
mkdir "$OUTPUT_DIR/multi-conf.full"
generate-multi-conf.sh -c common.simplified2.multi-conf.part -g genetic.simplified.multi-conf.part 0 "$OUTPUT_DIR/multi-conf.full"
cat conf/meta-template.simplified.multi-conf > "$OUTPUT_DIR/multi-conf.full/meta-template.multi-conf"
```
...
```{bash prepare.input}
source session-setup.sh
ls "$OUTPUT_DIR/multi-conf.full/*" | prepare-input-data.sh -l english -i $OUTPUT_DIR/impostors.full/my.impostors "$OUTPUT_DIR/input-data.full" "$OUTPUT_DIR/full"
```
